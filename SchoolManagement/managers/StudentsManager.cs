...
public class StudentsManager
{
    ...
    public void manage(MySqlConnection connection)
    {
        ...
        while (go)
        {
            string option = secondMenu("Student");
            switch (option)
            {
                case "1":
                    Console.WriteLine("Add a student");
                    MySqlCommand cmd = new MySqlCommand("INSERT INTO students(firstName,lastName,email,birthdate,address,phoneNr) VALUES(@v1,@v2,@v3,@v4,@v5,@v6);", connection);
                    ...
                    break;
                case "2":
                    cmd = new MySqlCommand("SELECT * FROM students", connection);
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (!reader.HasRows) Console.WriteLine("No data");
                            ...
                            while (reader.Read())
                            {
                                Console.WriteLine($"Student - {reader.GetInt32(0)}");
                    ...
                    break;
                case "3":
                    cmd = new MySqlCommand("SELECT * FROM students WHERE id = @id", connection);
                    int id;
                    do
                    {
                ...
                case "4":
                    ...
                    cmd = new MySqlCommand("UPDATE students\r\nSET firstName = @v1,\r\nlastName = @v2,\r\nemail = @v3,\r\nbirthdate = @v4,\r\naddress = @v5,\r\nphoneNr = @v6\r\nWHERE id = @id;", connection);
                    do
                    {
                        id = Convert.ToInt32(read("Enter id: "));
                    } while (!checkForId(id, connection));
                ...
                case "5":
                    try
                    {
                        ...
                        cmd = new MySqlCommand("DELETE FROM students WHERE id = @id", connection);
    ...
    {
        MySqlCommand command = new MySqlCommand("SELECT id FROM students WHERE id = @id", connection);
